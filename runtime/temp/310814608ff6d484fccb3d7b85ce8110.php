<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:75:"/Users/zyh/apache/thinkphp/public/../app/index/view/index/showschedule.html";i:1559026368;s:62:"/Users/zyh/apache/thinkphp/app/index/view/common/vertical.html";i:1558062664;}*/ ?>
<!doctype html>
<html lang="zh">
<head>
	<meta charset="UTF-8">
		...
		<style type="text/css">
			...
			#msform li{
				...
				<ul class="list-group"id="merform">
					...
					<input style="display: none" data-id="<?php echo $m['merchant_id']; ?>" value="<?php echo $m['merchant_name']; ?>" name = "merchant_id">
					<li class="list-group-item">
						<h2 class="fs-title">
					...
					<footer style="min-height: 80px;position: relative;bottom: 0;">
						...
						<?php  $founder_id = $schedule->getData('schedule_founder');	if($id == $founder_id): ?>
						<input type="button" name="button" class="submit action-button" onclick="window.location.href='/thinkphp/public/index/addparty/del?schedule_id=<?php echo $schedule['schedule_id']; ?>'" value="删除" />
						<?php endif; ?>
	...
	<script>
		...
		function save() {
			...
			var schedule_name = schedule[0].value,
					...
					schedule_id_morning = schedule[6].dataset.id,
					schedule_noon = schedule[7].value,
					schedule_id_noon = schedule[7].dataset.id,
					schedule_afternoon = schedule[8].value,
					schedule_id_afternoon = schedule[8].dataset.id,
					schedule_dinner = schedule[9].value,
					schedule_id_dinner = schedule[9].dataset.id;
			...
			sessionStorage.setItem('schedule_id_morning', schedule_id_morning);
			sessionStorage.setItem('schedule_noon', schedule_noon);
			sessionStorage.setItem('schedule_id_noon', schedule_id_noon);
			sessionStorage.setItem('schedule_afternoon', schedule_afternoon);
			sessionStorage.setItem('schedule_id_afternoon', schedule_id_afternoon);
			sessionStorage.setItem('schedule_dinner', schedule_dinner);
			sessionStorage.setItem('schedule_id_dinner', schedule_id_dinner);
