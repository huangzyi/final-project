...
//        {
        ...
        $data = ['favor_uid' => Session::get('id'), 'favor_content_id' => $content_id];
        ...
        if ($exists) {
            if (Db::table($table)->delete($data))
            {
                ...
                return $this->error('取消收藏成功');
            }
            else   return $this->error('取消收藏失败');
        } else {
            if (Db::table($table)->insert($data))
                return $this->error('收藏成功');
            else   return $this->error('收藏失败');
        }
    }
}