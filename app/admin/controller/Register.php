...
//            echo $type;
            if($type == "register")
                $user = User::get(['user_uname'=>$user->user_uname]);
                return $this->success('成功','/thinkphp/public/admin/index/information.html?user_id='.$user->user_id);
        } else {
    ...
    public function del($user_id)
    {
        ...
        {
            Db::table('group')->where('group_founder', $user_id)->update(['group_founder' => '2']);
            Db::table('schedule')->where('schedule_founder', $user_id)->update(['schedule_founder' => '2']);
            return $this->success('注销成功','/thinkphp/public/admin/index/finduser.html');
